    ...
    <AnimatePresence>
      {reveal ? (
        <motion.div
          ...
          className="absolute inset-0 z-40 flex items-center justify-center bg-pitch-950/92 backdrop-blur-sm"
        >
...
function IncomingCard() {
  return (
    ...
    >
      <div className="absolute inset-0 -translate-x-full animate-sheen bg-gradient-to-r from-transparent via-gold-400/20 to-transparent" />
      <div className="relative text-center">
        <p className="font-display text-tv-md font-semibold uppercase tracking-[0.5em] text-gold-500">
          Jorkan League
        </p>
        <h1 className="headline mt-[0.6rem] text-tv-4xl leading-[0.9] text-white">
          The Pick Is In
        </h1>
...
function PlayerCard({ reveal }: { reveal: RevealState }) {
  ...
  return (
    <motion.div
      ...
      className="relative flex w-[86rem] items-stretch gap-[2.4rem] rounded-[0.6rem] border border-white/12 bg-pitch-900/90 p-[2rem] shadow-panel"
    ...
    >
      ...
      <PlayerHeadshot
        ...
        className="relative h-[24rem] w-[19rem] shrink-0 border border-white/10"
      />
      ...
      <div className="relative flex min-w-0 flex-1 flex-col justify-between py-[0.3rem]">
        <div>
          ...
          <motion.h1
            ...
            className="headline mt-[0.5rem] break-words text-tv-3xl leading-[0.88] text-white"
          >
            {pick.player.name}
          </motion.h1>
          ...
          >
            ...
            />
            <span className="font-display text-tv-lg font-semibold uppercase tracking-[0.06em] text-white/80">
        ...
        <motion.div
          ...
          className="relative mt-[1.4rem] rounded-[0.35rem] border-l-[0.28rem] bg-white/[0.04] px-[1.2rem] py-[0.8rem]"
          style={{ borderLeftColor: accent }}
        >
          <p className="font-display text-tv-xs font-semibold uppercase tracking-[0.36em] text-white/45">
            Selected by
          </p>
          <div className="mt-[0.24rem] flex items-baseline gap-[0.9rem]">
            <h2 className="headline text-tv-xl leading-none text-white">{pick.fantasyTeamName}</h2>
            {pick.managerName ? (
              <span className="font-display text-tv-md uppercase tracking-[0.16em] text-white/50">
                {pick.managerName}
              </span>
            ) : null}
          </div>
