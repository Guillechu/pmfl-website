    ...
    <article className="card card-hover p-5 animate-fade-up">
      <div className="flex items-center justify-between mb-3">
        ...
        {isFinal ? (
          ...
          <span className="text-xs text-white/60">{formatGameDate(game.date)}</span>
        )}
      </div>
  ...
  return (
    <div className={cn("flex items-center gap-3", dimmed && "opacity-60")}>
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className={cn("font-display text-base text-white truncate", winner && "text-brand-gold-300")}>
          {team.name}
        </div>
      </div>
      <div className={cn("font-display text-2xl tabular-nums", winner ? "text-brand-gold-300" : "text-white")}>
        {score ?? "—"}
      </div>
    </div>
  );
